...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   display_list_contents.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: agama <marvin@42.fr>                       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/20 09:49:26 by agama             #+#    #+#             */
/*   Updated: 2018/08/31 17:48:50 by agama            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	get_file_acl(char path[PATH_MAX])
{
	...
	if (listxattr(path, buf, sizeof(buf), XATTR_NOFOLLOW) > 0)
		return ('@');
	if ((temp = acl_get_link_np(path, ACL_TYPE_EXTENDED)))
	{
		acl_free(temp);
		return ('+');
	}
...
int			display_list_contents(t_file *file, int size[7], int flags)
{
	char	str[12];
	char	buf[NAME_MAX + 1];
